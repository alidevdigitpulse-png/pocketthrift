...
class UpdateStoreRequest extends FormRequest
{
    ...
    {
        $store = $this->route('store'); // Get the store being updated
        
        return [
            'title'            => 'required|string|max:50',
            'category_id'      => 'nullable|exists:categories,id',
            'url_slug'         => ['required', 'string', 'max:255', function ($attribute, $value, $fail) use ($store) {
                $this->validateUniqueSlugPerRegion($value, $store->id, $fail);
            }],
            'active'           => 'boolean',
            'sort'             => 'integer|min:0',
