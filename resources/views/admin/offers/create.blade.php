                        ...
                        <form action="{{ route('admin.offer.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="seasonal_id" id="seasonal_id"
                                            class="form-control select2 @error('seasonal_id') is-invalid @enderror">
                                            <option value="">Select Seasonal Category (Optional)</option>
                                            @foreach ($seasonals as $seasonal)
                                                <option value="{{ $seasonal->id }}"
                                                    {{ old('seasonal_id') == $seasonal->id ? 'selected' : '' }}>
                                                    {{ $seasonal->title }}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <textarea name="affiliate_links" id="affiliate_links"
                                            class="form-control @error('affiliate_links') is-invalid @enderror" rows="3">{{ old('affiliate_links') }}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="terms_and_conditions" id="terms_and_conditions"
                                    class="form-control @error('terms_and_conditions') is-invalid @enderror" rows="4">{{ old('terms_and_conditions') }}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                @if (auth()->user()->role == 1 || auth()->user()->hasRole('admin') || auth()->user()->hasRole('super admin'))
                                    <select name="country_codes[]" id="country_codes"
                                        class="form-control select2 @error('country_codes') is-invalid @enderror" multiple>
                                        @foreach ($regions as $region)
                                            <option value="{{ $region->id }}"
                                                {{ in_array($region->id, old('country_codes', [])) ? 'selected' : '' }}>
                                ...
                                @else
                                    ...
                                    <input type="hidden" name="country_codes[]"
                                        value="{{ $regions->count() > 0 ? $regions->first()->id : '' }}">
                                @endif
                                @error('country_codes')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                            ...
                            <div class="form-group text-end">
                                <button type="submit" id="submitBtn" class="btn btn-primary">Create Offer</button>
                            </div>
                        </form>
                    </div>
                </div>
