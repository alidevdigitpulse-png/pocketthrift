    ...
    <div class="container-full">
        <div class="content-header">
            <div class="d-flex align-items-center">
                <div class="mr-auto">
                    <h3 class="page-title">{{ $data == null ? 'Add' : 'Update ' }} Customer
                        ...
                        <nav>
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ $data == null ? 'Add' : 'Update ' }} Customer</li>
                            </ol>
                        </nav>
                    </div>
                </div>
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="email"
                                                value="{{ $data == null ? old('email') : $data->email }}"
                                                class="form-control">
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="assigned_region" id="assigned_region" class="form-control select2">
                                                <option value="">Select a Region</option>
                                                @foreach($regions as $region)
                                                    <option value="{{ $region->code }}" {{ (old('assigned_region', $selectedRegion ?? '') == $region->code) ? 'selected' : '' }}>
                                                        {{ $region->country }} ({{ $region->code }})
                                                    </option>
                                                @endforeach
                                            </select>
                                            <small class="form-text text-muted">Select the region this user will have access to</small>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
...
@push('js')
    <script>
        $(document).ready(function() {
            // Initialize select2 for the assigned region single select
            if ($('#assigned_region').length) {
                setTimeout(function() {
                    $('#assigned_region').select2({
