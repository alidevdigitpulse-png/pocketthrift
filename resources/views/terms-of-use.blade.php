...
@php
    ...
    $baseUrl = url($regionPrefix . '/terms-of-use');
@endphp
@push('schemas')
...
@php
    ...
    $description = $meta['description'] ?? 'Read the Terms of Use for PocketThrift. Our guidelines ensure a secure and enjoyable experience. Learn about your rights, responsibilities, and how we manage your data.';
...
@php
    ...
    // Hardcode "Terms of Use" for this page
    $title = 'Terms of Use';
    $description = $meta['description'] ?? 'Read the Terms of Use for PocketThrift. Our guidelines ensure a secure and enjoyable experience. Learn about your rights, responsibilities, and how we manage your data.';
    $path = $meta['path'] ?? request()->getPathInfo();
...
<div class="container">
    <div class="row justify-content-center">
                ...
                <div class="card-body pt-3">
                    @if($page)
                        @php
                            // Look for most specific section names first
                            $sectionNames = [
                                'terms-content-' . $currentRegion,
                                'terms-' . $currentRegion,
                                'content-' . $currentRegion . '-terms',
                                'content-' . $currentRegion,
                                $currentRegion . '-terms-content',
                        ...
                        @if($content)
                            {!! $content !!}
                        @else
                            <p>Terms of use for this region do not have content yet.</p>
                        @endif
                    @else
                        <p>Terms of use not available for this region.</p>
                    @endif
                </div>
            </div>
                                @push('breadcrumb')
    ...
    <div class="border-top bg-white px-md-2 px-3 mt-4">
        <div class="container py-3">
            <ul class="list-unstyled d-flex align-items-center gap-2 mb-0">
                ...
                <li class="d-flex align-items-center">
                    <span class="fw-semibold">Terms of Use</span>
                </li>
            </ul>
        </div>
    </div>
