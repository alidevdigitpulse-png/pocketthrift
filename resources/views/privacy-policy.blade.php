...
@php
    ...
    $baseUrl = url($regionPrefix . '/privacy-policy');
@endphp
@push('schemas')
...
@php
    ...
    $pageName = $page->name ?? ucfirst(str_replace('-', ' ', $page->slug ?? ''));
    $pageName = preg_replace('/-' . $regionCode . '$/i', '', $pageName);
    $pageName = trim($pageName);
    
    $meta['title'] = $page->meta_title ?? $pageName;
    $title = $meta['title'] ?? ($pageName ?? ($pageTitle ?? ''));
    $description = $meta['description'] ?? 'To understand how we collect, use and protect your personal data, read Pocketthrift Privacy Policy. In essence, our main concern is your privacy.';
...
@section('content')
    <div class="policy-sec">
        <div class="container">
            <!--<h1 class="page-titles">Privacy Policy</h1>-->

            @if($page)
                @php
                    // Look for most specific section names first for privacy policy content
                    $sectionNames = [
                        'privacy-content-' . $currentRegion,
                        'privacy-' . $currentRegion,
                        'content-' . $currentRegion . '-privacy',
                        'content-' . $currentRegion,
                        $currentRegion . '-privacy-content',
                ...
                @if($content)
                    {!! $content !!}
                @else
                    <p>Privacy Policy content for this region does not have content yet.</p>
                @endif
            @else
                <p>Privacy Policy content not available for this region.</p>
            @endif
        </div>
                @push('breadcrumb')
