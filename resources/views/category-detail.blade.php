...
<style>
    ...
    @media (max-width: 767.98px) {
        ...
        .coupon-card .col-2:first-child,
        /* specific fix if col-2 is used in HTML */
        ...
        .coupon-card .col-8,
        /* specific fix if col-8 is used in HTML */
...
<div class="container py-3 px-md-2 px-3">
    <div class="row">
        <div class="col-lg-12 col-md-8 my-auto">
            <div class="">
                <div class="">
                    <h1 class="h2 mb-1" style="font-size:24px;font-weight:600;">{{ $categoryRecord->title }}
                        {{ __('Coupons & Promo Codes') }}, {{ date('Y') }}</h1>
                    <h2 class="mb-2" style="font-size:16px;font-weight:500;color:#cf5103;">
                        <span class="fw-semibold">{{ $categoryRecord->stores->count() }}</span>
                        {{ __('Best Coupons & Offers last validated on') }} <span>- {{ __(date('F')) }},
                            {{ date('Y') }}</span>
                    </h2>
    ...
    <div class="row">
        <div class="col-lg-3 col-md-4 sidebar order-2 order-md-1">
            ...
            <div class="widget widget-coupon">
                ...
                <ul>
                    @php
                        $allOffers = $categoryRecord->stores->flatMap(function ($store) {
                            return $store->offers;
                        });
                    @endphp
            ...
            <div class="widget">
                ...
                <div class="mt-3">
                    <p class="text-sm border rounded-md bg-light p-2">
                        <span class="fw-medium text-secondary">{{ $categoryRecord->title }}</span>
                    ...
                    <div class="coupon-card offer-item" data-type="{{ $offer->type }}">
                                    ...
                                    @if ($isFreeShipping)
                                        <p class="mb-2 fw-bold value">
                                            <i class="fa-solid fa-truck"
                                                style="font-size: 24px; margin-bottom: 2px;"></i>
                        ...
                        @endif
                            ...
                            <div class="col-8">
                                ...
                                <div>
                                    <h3 style="font-size: 16px; font-weight: 600; margin-top: 10px;">
                                        {{ $offer->store->title }} - {{ $offer->title }}</h3>
                                </div>
                            </div>
                            <div class="col-2">
                                ...
                                <div class="button-deck" style="float: right; margin-top: 10px;">
                                    @if ($offer->type == 'Code')
                                        <button class="reveal-code-button" data-bs-toggle="modal"
                                            ...
                                            data-offer-code="{{ $offer->button_text }}">{{ __('REVEAL') }}
                                            <br>{{ __('CODE') }}</button>
                                    @else
                                        <button class="deal-button" data-bs-toggle="modal"
                                            data-bs-target="#offerModal" data-offer-id="{{ $offer->id }}"
                                            data-offer-type="deal">{{ __('GET') }}
                                            <br>{{ __('DEAL') }}</button>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="offer-store-viewer">
                            <p>{{ __('View all offers of :store_title', ['store_title' => $offer->store->title]) }}
                                <a href="{{ $currentRegion === 'us' || !$currentRegion ? route('store.detail', ltrim($offer->store->url_slug, '/')) : route('region.store.detail', ['region' => $currentRegion, 'store' => ltrim($offer->store->url_slug, '/')]) }}"
                                    class="text-decoration-none">{{ __('here') }}</a>
                            </p>
                        </div>
                    </div>
        ...
        <div class="border-top bg-white px-md-2 px-3 mt-4">
            <div class="container py-3">
                <ul class="list-unstyled d-flex align-items-center gap-2 mb-0">
                    ...
                    <li class="d-flex align-items-center">
                        <a href="{{ $currentRegion === 'us' || !$currentRegion ? route('categories') : route('region.categories', ['region' => $currentRegion]) }}"
                            class="text-decoration-none">{{ __('Category') }}</a>
                        <svg fill="rgba(0,0,0,1)" height="18" viewBox="0 0 24 24" width="18"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M13.1717 12.0007L8.22192 7.05093L9.63614 5.63672L16.0001 12.0007L9.63614 18.3646L8.22192 16.9504L13.1717 12.0007Z">
            ...
            <div class="modal-body p-4">
                ...
                <div class="text-center">
                    ...
                    <div class="coupon-code-container mb-3">
                        <div class="d-flex align-items-center justify-content-center">
                            <span id="couponCode"
                                class="coupon-code flex-grow-1 text-center p-3 bg-light border border-secondary rounded fw-bold">LOADING...</span>
                ...
                .then(data => {
                    ...
                    if (isVerified) {
                        verifiedStatus.innerHTML =
                            "{{ __('Verified') }} <i class=\"fas fa-check-circle text-success\"></i> | {{ __('Valid Till') }}: " +
                            validTillText;
                    } else {
                        verifiedStatus.innerHTML =
                            "{{ __('Not Verified') }} | {{ __('Valid Till') }}: " + validTillText;
                    }
                    ...
                    if (offer.description) {
                        ...
                        if (isNumeric(offer.discount)) {
                            defaultItems.push(
                                `{{ __('Get') }} ${Math.abs(parseFloat(offer.discount))}% {{ __('off on your order.') }}`
                                );
                        }
