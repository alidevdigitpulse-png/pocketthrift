...
@php
    ...
    $description = $meta['description'] ?? $page->meta_description ?? 'Refer to the affiliate disclosure of PocketThrift if you wish to know how we generate commissions through the various affiliate links embedded in the content and the promise we make to remain fair and frank at all times.';
...
<div class="container">
    <div class="policy-sec">
        ...
        @if($page)
            @php
                // Look for most specific section names first for affiliate disclaimer content
                $sectionNames = [
                    'affiliate-content-' . $currentRegion,
                    'affiliate-' . $currentRegion,
                    'content-' . $currentRegion . '-affiliate',
                    'content-' . $currentRegion,
                    $currentRegion . '-affiliate-content',
            ...
            @if($content)
                {!! $content !!}
            @else
                <p>Affiliate Disclaimer content for this region does not have content yet.</p>
            @endif
        @else
            <p>Affiliate Disclaimer content not available for this region.</p>
        @endif
    </div>
     @push('breadcrumb')
    ...
    <div class="border-top bg-white px-md-2 px-3 mt-4">
        <div class="container py-3">
            <ul class="list-unstyled d-flex align-items-center gap-2 mb-0">
                ...
                <li class="d-flex align-items-center">
                    <span class="fw-semibold">Affiliate Disclaimer</span>
                </li>
            </ul>
        </div>
    </div>
